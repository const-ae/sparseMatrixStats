...
As you can see `sparseMatrixStats` is ca. 70 times fast than `matrixStats`, which in turn is 7 times faster than the `apply()` version.
...
Not all methods have yet been optimized for `dgCMatrix` input. The following list gives an overview which already have:
...
notes <- c("colAnyMissings"="Not implemented because it is deprecated",
...
api_df <- data.frame(
  ...
  DelayedMatrixStats = ifelse(matrixStats_functions %in% DelayedMatrixStats_functions, "✔", "❌"),
